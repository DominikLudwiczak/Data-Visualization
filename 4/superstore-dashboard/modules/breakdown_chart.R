...
ui <- function(id) {
  ...
  tagList(
    div(
      ...
      div(class = "item", "Profit split by category"),
    ),
    div(
...
server <- function(input, output, session, df, y, state) {
  output$breakdown <- renderPlot({
    ...
    df_bar <- df %>%
      ...
      mutate(category = fct_reorder(category, sum))

    bar <- ggplot(df_bar, aes(x = category, y = sum))
    bar +
      coord_flip() +
      geom_bar(
        fill = "#189ad3",
      ...
      theme(
        ...
        axis.text.x = element_text(color = "#6E757B"),
